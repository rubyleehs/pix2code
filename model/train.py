...
def run(input_path, output_path, is_memory_intensive=False, pretrained_model=None):
    ...
    if not is_memory_intensive:
        ...
        print(len(dataset.input_images), len(
            dataset.partial_sequences), len(dataset.next_words))
        ...
        generator = Generator.data_generator(
            voc, gui_paths, img_paths, batch_size=BATCH_SIZE, generate_binary_sequences=True)
    ...
    if not is_memory_intensive:
        model.fit(dataset.input_images,
...
if __name__ == "__main__":
    ...
    if len(argv) < 2:
        ...
        input_path = argv[0]
        output_path = argv[1]
        use_generator = False if len(
            argv) < 3 else True if int(argv[2]) == 1 else False
